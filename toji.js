...
if (conf.AUTO_REACT_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dj = '2349134457509';
            const dj2 = '2349134457509';
            const dj3 = "2349134457509";
            const luffy = '2349134457509';
            ...
            console.log("\t✅️TOJI-MD IS ONLINE✅️");
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*TOJI-MD WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                console.log("Toji md connected successfully✔");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Toji Md is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading Toji Md Commands ...\n");
                fs.readdirSync(__dirname + "/toji").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/toji/" + fichier);
                            console.log(fichier + " Installed Successfully✔️");
                        }
                        catch (e) {
                            console.log(`${fichier} could not be installed due to : ${e}`);
                        } /* require(__dirname + "/johan/" + fichier);
